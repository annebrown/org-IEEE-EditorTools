...
################################################################################
@txt_filenames = get_filenames(); # Get list of input files
print "\nList of .txt files:\n";
...
foreach (@txt_filenames) {
	...
	print "\nConverting filename: \"$_\" into \"$output_filename\"\n";
...
sub convert_txt_comment_to_csv {
	...
	while(<INFILE>){                   #reads line by line from FILE
		#Debug
		print "##############\nLine: $line_no"; $line_no ++; print "\"$_\"\n";
		
		# Remove newline from all but Comments and Remedys
		if (!$commentstart and !$remedystart) {chomp;}  #avoids \n on last field
		
		if (/CommentID:/) { $str = $str ."\"0";	next;} # begin comment} 
		...
		elsif ($commentstart or $remedystart) { $str = $str .$_; next; }  #avoids \n on last field
		else { print "Str: $str nexted\n"; next; }
