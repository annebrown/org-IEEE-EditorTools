...
sub convert_txt_comment_to_csv {
	...
	while(<INFILE>){                   #reads line by line from FILE 
		chomp;  #avoids \n on last field
		if ($_) { c$_ =~ s/^\s+//; $_ =~ s/\s+$//; print "\$_ after: $_";} # trim leading and trailing whitespaces
		...
		#$_ =~ s/^\s+|\s+$//g;
		#$_ =~ s/^\s+//; $_ =~ s/\s+$//;
		if ($_) {print "\$_ before: $_"; $_ =~ s/^\s+//; $_ =~ s/\s+$//;  print "\$_ after: $_";} # trim leading and trailing whitespaces
		#print "$_\n";
		if ($commentstart or $remedystart) { $str = $str ."$_"; } else { $str = $str ."~$_"; }		 
	}	
